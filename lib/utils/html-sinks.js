...
// Alpine's `x-html` holds JavaScript source, not HTML: its value is known only
// when that source is one string literal, and sanitized when it is one call.
const STRING_LITERAL = /^(?:'(?:[^'\\\n]|\\.)*'|"(?:[^"\\\n]|\\.)*"|`(?:[^`\\$]|\\.|\$(?!\{))*`)$/s

// The callee of `name(…)` when the call is the whole source
const soleCallee = (source) => {
  const match = /^([\w$]+(?:\s*\.\s*[\w$]+)*)\s*\(/.exec(source)
  ...
  for (let index = match[0].length - 1; index < source.length; index++) {
    ...
    if (quote) {
      ...
      return index === source.length - 1 ? match[1].replace(/\s+/g, '') : undefined
    }
  }
...
export const isSafeAlpineHtml = (source, sanitizers) => {
  const code = source.trim()

  return STRING_LITERAL.test(code) || sanitizers.has(soleCallee(code))
}

// htmx requests these URLs and swaps the response into the page as HTML.
// htmx reads each attribute with a `data-` prefix too.
export const HTMX_REQUESTS = new Set(['get', 'post', 'put', 'patch', 'delete']
  .flatMap(method => [`hx${ method }`, `datahx${ method }`]))
